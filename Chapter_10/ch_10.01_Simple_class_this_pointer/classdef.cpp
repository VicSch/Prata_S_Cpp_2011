...
const Simple &Simple::maxSalary(const Simple &s) const {
	if(s.salary > salary)
		return s;
	else
		return *this;	// 'this' is pointer to current object,
						// to get object from them we shoud derefrernce it
}

std::string Simple::getName() const {return name;}

double Simple::getSalary() const {return salary;}
